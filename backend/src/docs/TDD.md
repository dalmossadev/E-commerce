...
campaigns, customers, variant_history, audit_log, payments
```
...
| Etapa | Detalhe |
|-------|---------|
| Geração | GET /api/v1/orders/:id/pix — retorna payload BR Code + base64 |
| Expiração | 30 minutos |
| Confirmação | PATCH /api/v1/orders/:id/confirm-payment — apenas admin |
| Resultado | Status muda para PAID, paymentConfirmedAt preenchido |
...
| PIX QRCode + confirmação manual | 🔄 Em progresso |
...
| PixQRCodeDisplay | 🔄 Em progresso |
| Valor correto no QRCode PIX | 🔄 Em progresso |
| Skeleton loaders | 🔄 |
...
|----------|---------|--------|
| QRCode exibe R$ 0,00 | campo `total` não mapeado corretamente no OrderSchema | 1.6.0 |
| OrderSchema usa `totalValue` inexistente | banco usa `total`, `subtotal`, `discount` em camelCase | 1.6.0 |
| fulfillmentType em order_items | camelCase no banco — sem `name: 'snake_case'` | 1.6.0 |
...
- [ ] Implementar botão confirmar pagamento (admin)
...
- [ ] Audit final Lighthouse A11Y > 90
- [ ] Build produção
